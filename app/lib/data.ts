...
export async function getSchema(email: string) {
    try {
    ...
    `;
        ...
        return schemaResult.rows[0]?.schema; // Assuming each user has only one schema
    } catch (error) {
        console.error(
            "Failed to fetch schema for user email:",
            email,
            "Error:",
            error
        );
